...
export class DIDExchange {
   ...
   *
   * @param {string} invitation - out-of-band DIDComm invitation.
   * @param {Object} options - options for connect.
   * @param {string} options.waitForCompletion - wait for custom 'didexchange-state-complete' message to conclude connection as completed.
   * @param {string} options.label=agent-default-label - custom label to be provided.
   */
  //TODO save connection in wallet store
  async connect(
    invitation,
    { waitForCompletion = "", label = DEFAULT_LABEL } = {}
  ) {
