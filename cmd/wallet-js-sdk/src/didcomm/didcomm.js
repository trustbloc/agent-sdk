...
export class DIDComm {
     ...
     */
    async connect(auth, invitation = {}, {myLabel, routerConnections = [], reuseConnection, reuseAnyConnection = false, timeout, waitForCompletion} = {}) {
     ...
     */
    async initiateCredentialShare(auth, invitation = {}, {myLabel, routerConnections, reuseConnection, reuseAnyConnection = false, connectionTimeout},
                                  {from, timeout} = {}) {
        let {presentationRequest} = await this.wallet.proposePresentation(auth, invitation, {
                myLabel,
                routerConnections,
                reuseConnection,
                reuseAnyConnection,
     ...
     */
    async completeCredentialShare(auth, threadID, presentations, {
        controller,
        verificationMethod,
        created,
        domain,
        ...
        let _prove = async (presentation) => {
            return await this.wallet.prove(auth, {presentation}, {
                controller,
                verificationMethod,
                created,
                domain,
