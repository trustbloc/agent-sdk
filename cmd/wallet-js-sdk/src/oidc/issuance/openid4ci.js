...
export class OpenID4CI {
   ...
   *
   * @param{Object} agent - instance of the wallet user's agent initialized
   * @param{Object} user - id of the wallet user
   ...
   *
   */
  constructor({ agent, user, clientConfig }) {
    if (!agent) {
      throw new TypeError(
        "Error initializing OpenID4CI client: agent cannot be empty"
      );
    } else if (!user) {
      throw new TypeError(
        "Error initializing OpenID4CI client: user cannot be empty"
      );
    }
   ...
   */
  async authorize(authToken, kid, req, userPin = "") {
    const {
      issuer,
    ...
    if (preAuthCode && preAuthCode.length) {
      if (userPINRequired && (!userPin || !userPin.length)) {
        throw new Error(
          "Issuance Request indicates a user PIN is required, but no user PIN was provided."
        );
      }

      return await preAuthorized(
        authToken,
        kid,
        issuer,
        credentialType,
   ...
   *
   */
  async callback(callbackURI, clientState) {
    ...
    if (!authorizationCode || !authorizationCode.length) {
      throw new Error(
        "Callback URI is missing authorization `code` as a query parameter."
      );
    }

    if (!oauthState || !oauthState.length) {
      throw new Error(
        "Issuer did not include state parameter in callback URI when returning user to wallet."
      );
    }

    if (!clientState || !clientState.length) {
      throw new Error(
        "Cannot complete authorization flow, `callback` must be provided with clientState string returned by " +
    ...
    if (transactionData.oauthState !== oauthState) {
      throw new Error(
        "Issuer provided a state parameter that does not match the state parameter of the client-side transaction. " +
    ...
    const tokenResponse = await axios
      .post(transactionData.issuerMetadata.token_endpoint, tokenRequest)
      .then((resp) => resp.data)
      .catch((e) => {
        throw new Error(
          "Error authorizing wallet with Issuer: failed to exchange auth code for token:",
          e
        );
      });
...
async function pushAuthRequest(
  ...
  const { request_uri: requestURI } = await axios
    .post(issuerMetadata.pushed_authorization_request_endpoint, authRequest)
    .then((resp) => resp.data)
    .catch((e) => {
      throw new Error(
        "Error authorizing wallet with Issuer: failed to send Pushed Auth Request:",
        e
      );
    });
  ...
  return {
    redirect: redirectToIssuer.href,
    clientState: marshalTransaction(transactionData),
  };
}
...
async function preAuthorized(
  ...
  const tokenResponse = await axios
    ...
    .catch((e) => {
      throw new Error(
        "Error authorizing wallet with Issuer: failed to exchange pre-authorized code for token:",
        e
      );
    });
...
async function getCredential(
  ...
  const claims = {};
  claims["kid"] = kid;
  claims["iss"] = transactionData.clientID;
  claims["aud"] = transactionData.issuerMetadata.issuer;
  claims["iat"] = new Date().getTime() / 1000;
  claims["nonce"] = tokenResponse.c_nonce;

  const { jwt } = await jwtManager.signJWT(authToken, { claims, kid });
...
async function getIssuerMetadata(issuer_uri) {
  return await axios
    .get(issuer_uri + "/.well-known/openid-configuration")
    .then((resp) => {
      return resp.data;
    })
    .catch((e) => {
