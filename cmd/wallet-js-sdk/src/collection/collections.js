...
const JSONLD_CTX_COLLECTION = ['https://w3id.org/wallet/v1', 'https://trustbloc.github.io/context/wallet/collections-v1.jsonld']
const DEF_COLLECTION_TYPE = 'Vault'

var uuid = require('uuid/v4')

/**
 *  CollectionManager wallet collection data model features for grouping wallet contents.
 *
 *  This manager is useful for implementing credential vaults.
 *
...
export class CollectionManager {
     ...
     *  @param {String} collection.description - optional, custom collection type, default='ContentVault'.
     *
    ...
    async create(auth, {name, description, type = DEF_COLLECTION_TYPE} = {}) {
        ...
        await this.wallet.add({auth, contentType: contentTypes.COLLECTION, content})
     ...
     */
    async getAll(auth) {
        return await this.wallet.getAll({auth, contentType: contentTypes.COLLECTION})
    }
    ...
    async get(auth, collectionID) {
        return await this.wallet.get({auth, contentType: contentTypes.COLLECTION, contentID: collectionID})
    }
    ...
    async remove(auth, collectionID) {
        let {contents} = await this.wallet.getAll({auth, contentType: contentTypes.CREDENTIAL, collectionID})
        ...
        await Promise.all(
            [
                vcIDs.forEach(async (contentID) => await this.wallet.remove({auth, contentType:contentTypes.CREDENTIAL, contentID})),
                this.wallet.remove({auth, contentType: contentTypes.COLLECTION, contentID: collectionID})
            ]
        )
    }
    ...
    async update(auth, collectionID, {name, description} = {}) {
        ...
        let remove = this.wallet.remove({auth, contentType: contentTypes.COLLECTION, contentID: collectionID})
