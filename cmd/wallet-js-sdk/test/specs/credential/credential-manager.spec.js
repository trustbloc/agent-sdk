...
describe("Credential Manager data model tests", async function () {
  ...
  it("user saves a credential into wallet", async function () {
    ...
    collection = await collectionManager.create(auth, {
      name: "sample-name",
  ...
  it("user verified all credential related data models exists", async function () {
    const vcwallet = new UniversalWallet({
      agent: walletUserAgent,
      user: WALLET_USER,
    });

    let { contents } = await vcwallet.getAll({
      auth,
  ...
  it("user gets credential metadata by credential ID", async function () {
    ...
    let metadata = await credentialManager.getCredentialMetadata(
      auth,
      sampleUDC.id
    );
  ...
  it("user gets all credential metadata", async function () {
    ...
    metadataList = await credentialManager.getAllCredentialMetadata(auth, {
      credentialIDs: [sampleUDC.id, samplePRC.id],
    });
  ...
  it("user creates Orb DID in wallet and resolve it", async function () {
    ...
    let resolveDID = await didManager.resolveOrbDID(
      auth,
      docres.didDocument.id
    );
  ...
  it("user issues a credential from wallet", async function () {
    ...
    let { credential } = await credentialManager.issue(auth, samplePRC, {
      controller: did,
    });
  ...
  it("user verifies a credential stored in wallet", async function () {
    ...
    let { verified, error } = await credentialManager.verify(auth, {
      storedCredentialID: sampleUDC.id,
    });
  ...
  it("user presents credential from wallet", async function () {
    ...
    let { presentation } = await credentialManager.present(
      auth,
      {
  ...
  it("user derives a stored credential from wallet", async function () {
    ...
    let { credential } = await credentialManager.derive(
      auth,
      { storedCredentialID: sampleUDCBBS.id },
      {
        frame: sampleFrameDoc,
        nonce: uuidv4(),
      }
    );
    expect(credential).to.not.empty;
...
describe("Credential Query Tests", async function () {
  ...
  it("user performs mixed query in wallet - PresentationExchange, QueryByFrame, QueryByExample ", async function () {
    ...
    const metadataList = await Promise.all(
      credentialIDs.map(
        async (id) => await credentialManager.getCredentialMetadata(auth, id)
      )
    );
...
describe("Credential Manager blinded routing Manifest Credential Tests", async function () {
  ...
  it("user gets connection info from manifest", async function () {
    ...
    let mconnectionID = await credentialManager.getManifestConnection(
      auth,
      manifestVC.id
    );
  ...
  it.skip("user queries manifest credentials", async function () {
    ...
    manifest2.id = `http://example.gov/credentials/${uuidv4()}`;
    await credentialManager.saveManifestVC(auth, manifest2, uuidv4());
