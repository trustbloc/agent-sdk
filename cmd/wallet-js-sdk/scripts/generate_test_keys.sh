...
echo "Generating Wallet SDK Test PKI"
cd /opt/tmp
...
DNS.3 = mediator.example.com" >> "$tmp"
...
openssl req -new -key test/fixtures/keys/tls/ec-key.pem -subj "/C=CA/ST=ON/O=Example Inc.:Aries-Framework-Go/OU=Aries-Framework-Go/CN=localhost" -out test/fixtures/keys/tls/ec-key.csr
...
echo "done generating wallet SDK Test PKI"
